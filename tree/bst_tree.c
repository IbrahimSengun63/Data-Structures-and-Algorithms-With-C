...
void traversal(bts *tree) {
    ...
    // Eğer NULL ise:
    // - Bu çağrı biter
    // - Call stack'ten çıkar
    // - Bunu çağıran fonksiyonun kaldığı satıra dönülür
    if (tree == NULL) {
        return;
    }
...
void preorder(bts *tree) {
    ...
    printf("%d ", tree->data);     // KÖK
    preorder(tree->left);          // SOL
    preorder(tree->right);         // SAĞ
}

// Post-order traversal: SOL - SAĞ - KÖK
void postorder(bts *tree) {

    if (tree == NULL) {
        return;
    }

    postorder(tree->left);         // SOL
    postorder(tree->right);        // SAĞ
    printf("%d ", tree->data);     // KÖK
}
